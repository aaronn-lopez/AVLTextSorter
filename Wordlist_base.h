...
struct Wordlist_base
{
    ...
    //Returns the total number of words in the word list, i.e. the sum of the word counts
    virtual int total_words() const = 0;

    //Returns true if the words in the word list are in ascending sorted order, false otherwise
    //For an AVL implementation, this should return true if the AVL tree is a BST, and false otherwise
    virtual bool is_sorted() const = 0;

/*
    Returns a string of the most frequent word in the format "word freq".
    E.g. if the most frequent word is "the" with count 5437, then "the 5437" is returned. 
    
    If there is a tie, the first word in the list is returned. For an AVL implementation, 
    this means return the word that occurs first alphabetically.
...
*/
    ...
    //Adds w to the word list. If w is already in the word list, then increment its count
    //Otherwise add a new Node (with count 1) at the alphabetically correct location for the word
